...
evaluate_model <- function(
  ...
  probabilities <- as.numeric(model$predict(x_test, verbose = 0))
...
plot_history <- function(model_name, history) {
  ...
  png(
    file.path(PLOTS_DIR, paste0(model_name, "_loss.png")),
    width = 900,
    height = 600
  )
  ...
  png(
    file.path(PLOTS_DIR, paste0(model_name, "_accuracy.png")),
    width = 900,
    height = 600
  )
...
plot_confusion_matrix_for_best_model <- function(model, x_test, y_test) {
  ...
  png(
    file.path(PLOTS_DIR, "best_model_confusion_matrix.png"),
    width = 800,
    height = 700
  )
  
  image(
    t(matrix_values[nrow(matrix_values):1, ]),
    axes = FALSE,
    main = "Best Model Confusion Matrix"
  )
  ...
  text(
    ...
    labels = as.vector(t(matrix_values[2:1, ])),
    cex = 2
  )
